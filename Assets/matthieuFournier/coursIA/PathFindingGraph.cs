...
public static class PathFindingGraph
{
    ...
    public static String myCrawl(String destName, List<Node> path, List<List<Node>> pathList)
    {
        String returnValue = "no way ...";

        if (path.Last().name == destName)
        {
            return String.Join("->", path.Select(node => node.name).ToArray());
        }

        Destination[] dests = path.Last().destinations;
        if (dests == null || dests.Length == 0) return returnValue;
